...
const cloud = {
  ...
  save: (farm) =>
    fetch(CLOUD_URL, {
      ...
      body: JSON.stringify({ action: "save", farm, token: CLOUD_TOKEN }),
    }),
  remove: (id) =>
    fetch(CLOUD_URL, {
      method: "POST", mode: "no-cors",
...
export default function GreenFieldsMethane() {
  ...
  const saveFarm = async () => {
    ...
    if (cloud.enabled()) {
      ...
      try { await cloud.save(farm); await refreshFarms(); setSavedNote("Synced “" + name + "” to cloud"); }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (GMAPS_KEY) {
        try {
          ...
          const map = new g.Map(mapEl.current, {
            ...
            streetViewControl: true, fullscreenControl: true, mapTypeControl: true,
          });
      ...
      sat.addTo(map); // default to aerial — best for spotting the pond and paddocks
      L.control.layers({ Satellite: sat, Map: streets }, null, { collapsed: false, position: "topright" }).addTo(map);
      map.on("click", (e) => {
...
.gf-cards{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:20px 0;}
...
.gf-energyrow{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;}
...
.gf-farmcols{display:grid;grid-template-columns:1fr 1fr;gap:22px;}
@media(max-width:760px){.gf-farmcols{grid-template-columns:1fr;}}
.gf-farmsaverow{display:flex;gap:8px;margin-bottom:4px;}
.gf-farminput{flex:1;font-family:'Inter';font-size:14px;padding:10px 12px;border:1.5px solid var(--line);
...
.gf-searchinput{flex:1;font-family:'Inter';font-size:14px;padding:10px 12px;border:1.5px solid var(--line);
  border-radius:10px;background:#fff;color:var(--ink);outline:none;}
...
.gf-map{height:230px;border-radius:12px;border:1px solid var(--line);background:#e9ede3;overflow:hidden;z-index:0;}
...
.gf-seasonstats{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:16px 0 4px;}
