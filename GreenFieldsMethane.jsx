...
const ECAN_LAYERS = [37, 38, 36]; // area, global, points (points tried last)
const ECAN_FIELDS = [
...
async function lookupConsent(lat, lng) {
  ...
  for (const layer of ECAN_LAYERS) {
    // points layer needs a search tolerance; polygons use a plain intersect
    const dist = layer === 36 ? "&distance=1500&units=esriSRUnit_Meter" : "";
    const url = ECAN_BASE + layer + "/query?f=json&geometry=" + geom +
      ...
      dist + "&outFields=" + ECAN_FIELDS + "&returnGeometry=false&resultRecordCount=5";
    try {
      const r = await fetch(url);
      if (!r.ok) continue;
      const d = await r.json();
      if (d && d.features && d.features.length) {
        return d.features.map((ft) => ft.attributes);
      }
    } catch (err) { /* try next layer */ }
  }
  return [];
}
              ...
              <div className="gf-consent">
                ...
                {!consentBusy && consents && consents.length === 0 && (
                  <div className="gf-consentnone">
                    No active ECan dairy effluent consent found at this pin. Try dropping the pin
                    closer to the dairy shed / effluent area, or enter details manually.
                  </div>
                )}
                    ...
                    <div className="gf-consentcard">
                      <div className="gf-consenthead">
                        <div>
                          ...
                          <span className="gf-consenttag">ECan dairy effluent</span>
                        </div>
                        {consents.length > 1 && (
