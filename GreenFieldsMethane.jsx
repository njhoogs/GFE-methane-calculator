...
const ECAN_FIELDS = [
  "ConsentNo", "ConsentStatus", "HolderName", "Location", "Expires", "fmDate",
...
// Public business-register lookups for a consent holder. NZBN covers companies,
// trusts, partnerships and sole traders (most farm entities are trusts, so it's
// the better first stop); the Companies Register covers registered companies.
// These give a registered/postal address and directors — NOT phone or email.
const nzbnSearchUrl = (name) =>
  "https://www.nzbn.govt.nz/mynzbn/search/?q=" + encodeURIComponent(name);
const companiesSearchUrl = (name) =>
  "https://app.companiesoffice.govt.nz/companies/app/ui/pages/companies/search?q=" +
  encodeURIComponent(name) + "&entityTypes=ALL&entityStatusGroups=ALL&mode=standard";

/* ------------------------------------------------------------------ *
 *  BULK EXPORT — every active Canterbury dairy effluent consent
 *  Pages through the public ECan open-data service and builds a CSV.
 *  Includes a per-holder NZBN / Companies Register lookup LINK so each
 *  one can be checked individually. It deliberately does NOT scrape
 *  personal contact data: the Companies Office terms forbid automated
 *  extraction of the register, and using director/shareholder personal
 *  data for unsolicited marketing breaches the Privacy Act 2020.
 *  For a bulk join, request Companies Office / NZBN bulk data access
 *  (free, monthly CSVs, under their Bulk Data Access Agreement).
 * ------------------------------------------------------------------ */
function centroidOf(geom) {
  if (!geom) return { lat: "", lng: "" };
  if (typeof geom.x === "number") return { lat: +geom.y.toFixed(5), lng: +geom.x.toFixed(5) };
  const ring = geom.rings && geom.rings[0];
  if (ring && ring.length) {
    let sx = 0, sy = 0;
    ring.forEach((p) => { sx += p[0]; sy += p[1]; });
    return { lat: +(sy / ring.length).toFixed(5), lng: +(sx / ring.length).toFixed(5) };
  }
  return { lat: "", lng: "" };
}
...
// ArcGIS returns dates as epoch milliseconds. (toDateText is a free-text
// DESCRIPTION field — it often just says "expiry date" — so never use it as a date.)
function fmtDate(v) {
...
async function fetchAllConsents(onProgress) {
  const PAGE = 1000;
  const seen = new Set();
  const rows = [];
  for (const layer of [37, 5, 38]) {
    let offset = 0;
    for (let guard = 0; guard < 40; guard++) {
      const url = ECAN_BASE + layer + "/query?f=json&where=1%3D1&outFields=*" +
        "&returnGeometry=true&outSR=4326&resultOffset=" + offset + "&resultRecordCount=" + PAGE;
      let j;
      try {
        const r = await fetch(url);
        if (!r.ok) break;
      ...
      feats.forEach((ft) => {
        ...
        const key = (a.ConsentNo || "") + "|" + (a.HolderName || "");
        if (!a.ConsentNo || seen.has(key)) return;
        seen.add(key);
        const c = centroidOf(ft.geometry);
        rows.push(Object.assign({}, a, {
          _expires: fmtDate(a.Expires),
          _from: fmtDate(a.fmDate),
          _lat: c.lat, _lng: c.lng,
          _layer: layer === 37 ? "disposal area" : layer === 5 ? "point" : "global area",
          _nzbn: a.HolderName ? nzbnSearchUrl(a.HolderName) : "",
          _companies: a.HolderName ? companiesSearchUrl(a.HolderName) : "",
        }));
      });
...
function consentColumns(rows) {
  const first = [
    "ConsentNo", "HolderName", "ConsentStatus", "Location", "_expires", "_from",
    "ConsentedAnimalNumbers", "ActualAnimalNumbers", "CowsWintered", "WinterMilking",
    "MilkingFrequency", "StorageMethod", "StorageLiner", "StorageVolume_m3",
    "DisposalMethod", "FeedPad", "DairyHandstandAdequate", "EfluentSolidsContained",
    "NutrientLoading", "FarmNumber", "GIS_TerritorialAuthority",
    "_lat", "_lng", "_layer", "LINK", "_nzbn", "_companies",
  ];
  const skip = ["SHAPE", "Expires", "fmDate", "toDate", "Lapses", "GivenEffectTo"];
  const rest = [];
  rows.forEach((r) => Object.keys(r).forEach((k) => {
    if (!first.includes(k) && !rest.includes(k) && !skip.includes(k)) rest.push(k);
  }));
  const cols = first.concat(rest);
  const label = (c) =>
    c === "_lat" ? "Latitude" : c === "_lng" ? "Longitude" : c === "_layer" ? "Record type" :
    c === "_nzbn" ? "NZBN lookup" : c === "_companies" ? "Companies Register lookup" :
    c === "_expires" ? "Expires" : c === "_from" ? "Granted" : c;
  return { cols, header: cols.map(label) };
}

async function exportAllConsents(onProgress) {
  const rows = await fetchAllConsents(onProgress);
  if (!rows.length) return { count: 0 };
  const { cols, header } = consentColumns(rows);
  ...
  a.download = "ecan-dairy-effluent-consents-" + new Date().toISOString().slice(0, 10) + ".csv";
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
  setTimeout(() => URL.revokeObjectURL(a.href), 4000);
  return { count: rows.length };
}

// Push the whole consent list into the Google Sheet (its own tab), in batches
// so we stay inside Apps Script's payload limits.
async function pushConsentsToSheet(onProgress) {
  const rows = await fetchAllConsents((n) => onProgress && onProgress("Fetched " + n + " consents…"));
  if (!rows.length) return { count: 0 };
  const { cols, header } = consentColumns(rows);
  const BATCH = 150;
  for (let i = 0; i < rows.length; i += BATCH) {
    const chunk = rows.slice(i, i + BATCH).map((r) => cols.map((c) => (r[c] === null || r[c] === undefined ? "" : r[c])));
    await fetch(CLOUD_URL, {
      method: "POST", mode: "no-cors",
      headers: { "Content-Type": "text/plain;charset=utf-8" },
      body: JSON.stringify({
        action: "consents",
        reset: i === 0, // first batch clears the tab and writes the header
        header, rows: chunk, token: CLOUD_TOKEN,
      }),
    });
    if (onProgress) onProgress("Sent " + Math.min(i + BATCH, rows.length) + " of " + rows.length + " to the Sheet…");
  }
  return { count: rows.length };
}
...
export default function GreenFieldsMethane() {
  ...
  const runExport = async () => {
    setExportBusy(true);
    setExportNote("Downloading consents from ECan…");
    try {
      const res = await exportAllConsents((n) => setExportNote("Fetched " + n + " consents…"));
      setExportNote(res.count ? "Exported " + res.count + " consents to CSV." : "No consents returned — ECan may have blocked the request.");
    } catch (e) {
      setExportNote("Export failed — ECan may have blocked the request from the browser.");
    } finally {
      setExportBusy(false);
  ...
  const runSheetPush = async () => {
    setExportBusy(true);
    setExportNote("Fetching consents…");
    try {
      const res = await pushConsentsToSheet((msg) => setExportNote(msg));
      setExportNote(res.count
        ? "Sent " + res.count + " consents to the 'consents' tab of your Google Sheet."
        : "No consents returned — ECan may have blocked the request.");
    } catch (e) {
      setExportNote("Couldn't send to the Sheet — check the cloud connection.");
    } finally {
      setExportBusy(false);
      setTimeout(() => setExportNote(""), 10000);
    }
  };
    ...
    <div className="gf">
      ...
      <section className="gf-panel gf-farmbar">
                ...
                {!consentBusy && consents && consents.length > 0 && (() => {
                  ...
                  return (
                    <div className="gf-consentcard">
                      ...
                      {!blank(c.HolderName) && (
                        <div className="gf-holder">
                          <div className="gf-holderlbl">Look up the holder</div>
                          <div className="gf-holdername">
                            <span>{String(c.HolderName)}</span>
                            <button
                              className="gf-copybtn"
                              onClick={() => {
                                try { navigator.clipboard.writeText(String(c.HolderName)); setCopied(true); setTimeout(() => setCopied(false), 1800); } catch (e) {}
                              }}
                            >{copied ? "Copied" : "Copy"}</button>
                          </div>
                          <div className="gf-holderlinks">
                            <a href={nzbnSearchUrl(String(c.HolderName))} target="_blank" rel="noopener noreferrer">NZBN register ↗</a>
                            <a href={companiesSearchUrl(String(c.HolderName))} target="_blank" rel="noopener noreferrer">Companies Register ↗</a>
                          </div>
                          <p className="gf-holdernote">
                            These public registers give a registered/postal address and directors — not a
                            phone number or email. NZBN is the better first stop for farms (it covers trusts
                            and partnerships, not just companies).
                          </p>
                        </div>
                      )}
        ...
        <div className="gf-export">
          <div className="gf-exportmain">
            <div className="gf-exportlbl">Bulk export</div>
            <p className="gf-exporttxt">
              Download <strong>every active Canterbury dairy effluent consent</strong> as a CSV —
              holder, storage method, liner, volume, disposal method, herd numbers, feed pad, expiry
              and map coordinates. Each row includes a one-click NZBN / Companies Register lookup
              link for that holder.
            </p>
          </div>
          <div className="gf-exportact">
            <div className="gf-exportbtns">
              {cloud.enabled() && (
                <button className="gf-sheetbtn" onClick={runSheetPush} disabled={exportBusy}>
                  {exportBusy ? "Working…" : "Send to Google Sheet"}
                </button>
              )}
              <button className="gf-exportbtn" onClick={runExport} disabled={exportBusy}>
                {exportBusy ? "Working…" : "Download CSV"}
              </button>
            </div>
            {exportNote && <div className="gf-exportnote">{exportNote}</div>}
          </div>
        </div>
      </section>
...
/* bulk export */
.gf-export{display:flex;align-items:center;justify-content:space-between;gap:20px;flex-wrap:wrap;
  margin-top:20px;padding-top:16px;border-top:1px solid var(--line);}
.gf-exportmain{flex:1;min-width:260px;}
.gf-exportlbl{font-family:'Space Grotesk';font-weight:600;font-size:12px;text-transform:uppercase;
  letter-spacing:.13em;color:var(--flare-deep);margin-bottom:6px;}
.gf-exporttxt{font-size:12.5px;color:var(--ink2);margin:0;line-height:1.5;max-width:560px;}
.gf-exportact{flex:none;display:flex;flex-direction:column;align-items:flex-end;gap:6px;}
.gf-exportbtns{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end;}
.gf-sheetbtn{font-family:'Space Grotesk';font-weight:600;font-size:13.5px;padding:11px 18px;border:none;
  border-radius:10px;background:var(--pasture);color:#fff;cursor:pointer;white-space:nowrap;}
.gf-sheetbtn:hover{background:#25583a;}
.gf-sheetbtn:disabled{opacity:.6;cursor:default;}
.gf-exportbtn{font-family:'Space Grotesk';font-weight:600;font-size:13.5px;padding:11px 18px;border:none;
  border-radius:10px;background:var(--ink);color:#f4f6ef;cursor:pointer;white-space:nowrap;}
.gf-exportbtn:hover{background:#0e1712;}
.gf-exportbtn:disabled{opacity:.6;cursor:default;}
.gf-exportnote{font-size:11.5px;color:var(--ink2);text-align:right;}
  border-radius:10px;background:#fff;color:var(--ink);outline:none;}
...
.gf-holder{margin-top:14px;padding-top:12px;border-top:1px dashed #d7e6ef;}
.gf-holderlbl{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.1em;
  color:var(--ink2);margin-bottom:7px;}
.gf-holdername{display:flex;align-items:center;justify-content:space-between;gap:10px;
  background:#fff;border:1.5px solid var(--line);border-radius:9px;padding:8px 10px;}
.gf-holdername span{font-size:13px;font-weight:600;color:var(--ink);word-break:break-word;}
.gf-copybtn{flex:none;font-family:inherit;font-size:11.5px;font-weight:600;color:var(--ink2);
  background:var(--paper);border:1px solid var(--line);border-radius:7px;padding:5px 10px;cursor:pointer;}
.gf-copybtn:hover{border-color:var(--brand-blue);color:var(--brand-blue);}
.gf-holderlinks{display:flex;gap:14px;flex-wrap:wrap;margin-top:9px;}
.gf-holderlinks a{font-size:12.5px;font-weight:600;color:var(--brand-blue);text-decoration:none;}
.gf-holderlinks a:hover{text-decoration:underline;}
.gf-holdernote{font-size:11.5px;color:var(--ink2);margin:9px 0 0;line-height:1.45;}
